...
public class PlayerHandler {
    ...
    public void applyCape(NativeImage capeImage) {
        ...
        if(capeImage.getHeight() != capeImage.getWidth() / 2) {
            ...
            capeImage.close();
            this.applyTexture(ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "capes/" + uuid), imgNew);
    ...
    public void applyEars(NativeImage earImage) {
        applyTexture(ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "ears/" + uuid), earImage);
    ...
    public void removeCape() {
        ...
        Minecraft.getInstance().execute(() -> {
            Minecraft.getInstance().getTextureManager().release(ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "capes/" + uuid));

            for (int i = 0; i < getAnimatedCape().size() - 1; i++) {
                Minecraft.getInstance().getTextureManager().release(ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, String.format("capes/%s/%d", uuid, i)));
            }
        });
    }
    ...
    public void removeEars() {
        ...
        Minecraft.getInstance().execute(() -> Minecraft.getInstance().getTextureManager().release(ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "ears/" + uuid)));
    }
    ...
    private void loadFramesToResource() {
        ...
        getAnimatedCape().forEach((integer, nativeImage) -> {
            ResourceLocation currentResource = ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, String.format("capes/%s/%d", uuid, integer));
    ...
    private ResourceLocation getFrame() {
        ...
        if(time > lastFrameTime + capeInterval) {
            ...
            return ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, String.format("capes/%s/%d", uuid, currentFrameNo));
        }
        return ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, String.format("capes/%s/%d", uuid, lastFrame));
    }
    ...
    public ResourceLocation getCapeLocation() {
        return hasStaticCape ? ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "capes/" + uuid) : hasAnimatedCape ? getFrame() : null;
    }
    ...
    public ResourceLocation getEarLocation() {
        return hasEars ? ResourceLocation.fromNamespaceAndPath(MinecraftCapes.MOD_ID, "ears/" + uuid) : null;
    }
